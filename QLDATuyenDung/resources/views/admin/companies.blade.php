...
<script>
        ...
        <tr>
            ...
            <td>
                <div style="display: flex; gap: 5px; flex-wrap: wrap;">
                    ...
                    <button class="btn btn-danger btn-delete-company" data-company-id="${company.id}" data-company-name="${company.company_name.replace(/"/g, '&quot;')}" style="padding: 6px 12px; font-size: 12px;">
    ...
    async function filterCompanies() {
        ...
        `;
            ...
            console.log('🔍 Đang tìm kiếm/lọc công ty...', {
                searchTerm,
                statusFilter
            });
    ...
    async function deleteCompany(companyId, companyName) {
        ...
        try {
            ...
            const preResp = await fetch(`/admin/companies/${companyId}`, {
                credentials: 'same-origin'
            });
            if (!preResp.ok) {
            ...
            if (siteAdmins.length > 0) {
                ...
                const confirmed = confirm(`Cảnh báo: Xóa công ty sẽ có thể hạ các tài khoản sau từ employer -> candidate: ${names}.\n\nBạn có chắc muốn tiếp tục không?`);
                if (!confirmed) {
                    return;
                }
            } else {
                // Không có member rủi ro đặc biệt -> confirm bình thường
                const confirmed = confirm(`Bạn có chắc chắn muốn xóa công ty "${companyName}"?\n\nHành động này sẽ:\n- Xóa quyền quản lý công ty\n- Tài khoản vẫn tồn tại nhưng mất quyền liên quan`);
                if (!confirmed) {
                    return;
                }
            }
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target.closest('.btn-delete-company')) {
            const btn = e.target.closest('.btn-delete-company');
            const companyId = btn.dataset.companyId;
            const companyName = btn.dataset.companyName;
            if (companyId && companyName) deleteCompany(companyId, companyName);
        }
    });
