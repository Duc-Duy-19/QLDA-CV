            ...
            <td>
                <div style="display: flex; gap: 5px; flex-wrap: wrap;">
                    ...
                    ${company.status === 'pending' ? `
                        <form action="/admin/companies/${company.id}/approve" method="POST" style="display: inline;">
                            @csrf
                            <button type="submit" class="btn btn-success" style="padding: 6px 12px; font-size: 12px;" onclick="return confirm('Duyệt công ty ${company.company_name}?')">
                                <i class="fas fa-check"></i> Duyệt
                            </button>
                        </form>
                        <form action="/admin/companies/${company.id}/reject" method="POST" style="display: inline;">
                            @csrf
                            <button type="submit" class="btn btn-danger" style="padding: 6px 12px; font-size: 12px;" onclick="return confirm('Từ chối công ty ${company.company_name}?')">
