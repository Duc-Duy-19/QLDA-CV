...
function loadCompanyData() {
    ...
    if (DEBUG_MODE) {
        ...
        console.log(
            "Token result:",
            token ? `Found: ${token.substring(0, 30)}...` : "Not found"
        );
    ...
    if (!token) {
        ...
        if (currentUser) {
            ...
            if (DEBUG_MODE)
                console.log("Using fallback mock data for user:", user.name);
    ...
    fetch(`${API_BASE_URL}/my-company`, {
        ...
        .then((data) => {
            ...
            if (data.company) {
                ...
                if (DEBUG_MODE)
                    console.log("Found company in data.data.company");
            } else if (data.data) {
                ...
                companyData = data;
                if (DEBUG_MODE)
                    console.log("Response is direct company object");
            }
            ...
            if (companyData) {
                    ...
                    try {
                        ...
                        if (cu) {
                            try {
                                ...
                                console.warn(
                                    "Could not update currentUser in localStorage",
                                    e
                                );
                            }
                        }
                        ...
                        try {
                            ...
                            if (companyData.company_name) {
                                document.title =
                                    companyData.company_name + " - WebCV";
                            }
                        } catch (e) {
                            console.warn(
                                "Could not update all UI company name placeholders",
                                e
                            );
                        }
                    } catch (e) {
                        console.warn(
                            "Error while normalizing company authoritative data",
                            e
                        );
                    }
                } else {
                    console.warn(
                        "Company object is empty or missing essential data"
                    );
                    throw new Error(
                        "Không tìm thấy thông tin công ty. User có thể chưa thuộc công ty nào hoặc công ty chưa được duyệt."
                    );
                }
            } else {
                ...
                throw new Error(
                    data.error ||
                        data.message ||
                        "Không tìm thấy dữ liệu công ty trong response"
                );
            }
        })
        .catch((error) => {
            ...
            if (error.message.includes("chưa thuộc công ty")) {
                ...
                showErrorMessage(
                    error.message || "Không thể tải thông tin công ty"
                );
            }
        });
}
...
function updateCompanyInfo(company) {
    ...
    if (companyName)
        companyName.textContent = company.company_name || "Chưa cập nhật";
    ...
    if (companySize) {
        ...
        if (employeeCount > 0) {
            if (employeeCount < 50) sizeText = "1-50 nhân viên";
                ...
                sizeText = `${employeeCount.toLocaleString("vi-VN")} nhân viên`;
        }
    ...
    if (companyAddress)
        companyAddress.textContent = company.address || "Chưa cập nhật địa chỉ";
    ...
    if (companyEmail)
        companyEmail.textContent = company.email || "Chưa cập nhật email";
    ...
    // Update company context (AI)
    if (company.company_context) {
        displayCompanyContext(company.company_context);
    }
}
...
function testTokenValidation() {
    ...
    fetch(`${API_BASE_URL}/my-company`, {
        ...
        .then((response) => {
            console.log(
                `Token Test - Status: ${response.status} ${response.statusText}`
            );
            if (response.status === 401) {
...
function fillCurrentData() {
    ...
    document.getElementById("editCompanyName").value =
        companyName !== "Chưa cập nhật" ? companyName : "";
    // Fill description unless it's a placeholder/loading text
    const descText = (companyDescription || "").toString().trim();
    const isPlaceholder =
        descText === "Đang tải thông tin công ty..." ||
        descText === "Chưa có mô tả công ty" ||
        descText === "";
    document.getElementById("editDescription").value = isPlaceholder
        ? ""
        : descText;
    document.getElementById("editAddress").value =
        companyAddress !== "Chưa cập nhật địa chỉ" ? companyAddress : "";
    document.getElementById("editPhone").value =
        companyPhone !== "Chưa cập nhật số điện thoại" ? companyPhone : "";
    document.getElementById("editEmail").value =
        companyEmail !== "Chưa cập nhật email" ? companyEmail : "";
    document.getElementById("editWebsite").value =
        companyWebsite !== "Chưa cập nhật website" ? companyWebsite : "";
}
...
function saveCompanyInfo() {
    ...
    if (logoFile && logoFile.size > 0) {
        ...
        if (!allowedTypes.includes(logoFile.type)) {
            showErrorMessage(
                "Định dạng logo không hợp lệ. Vui lòng chọn file hình ảnh (jpg, png, gif)"
            );
            saveBtn.innerHTML = "Lưu thay đổi";
            return;
        }
        if (logoFile.size > 2 * 1024 * 1024) {
            showErrorMessage(
                "Kích thước logo quá lớn. Vui lòng chọn file nhỏ hơn 2MB"
            );
            saveBtn.innerHTML = "Lưu thay đổi";
            return;
        }
        ...
        if (DEBUG_MODE) {
            try {
                ...
                console.debug("Could not iterate FormData entries", e);
            }

            console.log(
                "Sending PUT request (FormData) to update company, includes logo file"
            );
        }
        ...
        fetch(`${API_BASE_URL}/companies/${window.currentCompanyId}`, {
                ...
                if (!response.ok) {
                    if (response.status === 401) {
                        ...
                        return response.json().then((data) => {
                            console.error(
                                "Validation failed (422) response body:",
                                data
                            );
            ...
            .then((data) => {
                ...
                if (data.message && data.company) {
                    ...
                    showSuccessMessage(
                        data.message || "Cập nhật thông tin công ty thành công!"
                    );
                } else {
                    throw new Error(
                        data.error ||
            ...
            .catch((error) => {
                ...
                showErrorMessage(
                    error.message || "Không thể cập nhật thông tin công ty"
                );
            })
            .finally(() => {
                saveBtn.innerHTML = "Lưu thay đổi";
    ...
    if (DEBUG_MODE)
        console.log("Sending PUT request to update company (JSON):", payload);
    ...
    fetch(`${API_BASE_URL}/companies/${window.currentCompanyId}`, {
        ...
        .then((data) => {
            ...
            if (data.message && data.company) {
                ...
                throw new Error(
                    data.error || data.message || "Không thể cập nhật thông tin"
                );
            }
        })
        .catch((error) => {
...
function showMockCompanyData(user) {
    if (DEBUG_MODE)
        console.log("Showing mock company data for user:", user.name);
    ...
    const mockCompany = {
        ...
        description:
            "Công ty phần mềm hàng đầu Việt Nam, cung cấp dịch vụ phát triển phần mềm và giải pháp công nghệ",
    ...
    if (companyDescription) {
        companyDescription.innerHTML = `
            <div style="background: #fff3cd; border: 1px solid #ffeaa7; padding: 10px; border-radius: 4px; margin-bottom: 10px;">
                ...
                <small>Token hiện tại: ${
                    authToken ? authToken.substring(0, 20) + "..." : "Không có"
                }</small><br>
...
// ============================================
// COMPANY CONTEXT (AI) FUNCTIONS
// ============================================

function editCompanyContext() {
    ...
    // Load current context if exists
    const token = getAuthToken();
    fetch(`${API_BASE_URL}/my-company`, {
        method: "GET",
        headers: {
        ...
        .then((data) => {
            if (data.company && data.company.company_context) {
                const context = data.company.company_context;
                document.getElementById("context_culture").value =
                    context.culture || "";
                document.getElementById("context_values").value = Array.isArray(
                    context.values
                )
                    ? context.values.join("\n")
                    : context.values || "";
                document.getElementById("context_work_style").value =
                    context.work_style || "";
                document.getElementById("context_general_requirements").value =
                    context.general_requirements || "";
            }
            // Show modal
            document.getElementById("companyContextModal").style.display =
                "flex";
        })
        .catch((error) => {
...
function closeCompanyContextModal() {
    document.getElementById("companyContextModal").style.display = "none";
}
...
function saveCompanyContext() {
    ...
    const culture = document.getElementById("context_culture").value.trim();
    const valuesText = document.getElementById("context_values").value.trim();
    const workStyle = document
        .getElementById("context_work_style")
        .value.trim();
    const generalReqs = document
        .getElementById("context_general_requirements")
        .value.trim();
    ...
    const contextData = {
        company_context: {
            ...
            general_requirements: generalReqs,
        },
    };
    ...
    fetch(`${API_BASE_URL}/companies/${companyId}/context`, {
        method: "PUT",
        headers: {
            "Content-Type": "application/json",
            Authorization: token,
        ...
        .then((data) => {
            showToast("Cập nhật ngữ cảnh AI thành công!", "success");
            closeCompanyContextModal();
            // Reload to display new context
            loadCompanyData();
        })
        .catch((error) => {
            ...
            alert(
                "Lỗi: " +
                    (error.error || error.message || "Không thể lưu ngữ cảnh")
            );
        });
}

// Display company context in the info page
function displayCompanyContext(context) {
    const displayDiv = document.getElementById("company-context-display");
    ...
    let html = '<div class="context-display">';

    if (context.culture) {
        html += `
            <div class="context-item">
                <strong><i class="fas fa-building"></i> Văn hóa công ty:</strong>
                <p>${context.culture}</p>
            </div>`;
    }

    if (context.values && context.values.length > 0) {
        const valuesList = Array.isArray(context.values)
            ? context.values
        ...
        html += `
            <div class="context-item">
                <strong><i class="fas fa-star"></i> Giá trị cốt lõi:</strong>
                <div>${valuesList}</div>
            </div>`;
    }

    if (context.work_style) {
        html += `
            <div class="context-item">
                <strong><i class="fas fa-briefcase"></i> Phong cách làm việc:</strong>
                <p>${context.work_style}</p>
            </div>`;
    }

    if (context.general_requirements) {
        html += `
            <div class="context-item">
                <strong><i class="fas fa-check-circle"></i> Yêu cầu chung:</strong>
                <p>${context.general_requirements}</p>
            </div>`;
    }
