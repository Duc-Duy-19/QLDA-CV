...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (!companyId) {
        root.innerHTML =
            '<div class="alert alert-danger">ID công ty không hợp lệ.</div>';
        return;
    }
    ...
    fetch(`/api/public/companies/${companyId}`)
        ...
        .then((data) => {
            ...
            <div class="company-hero">
                <div class="company-logo"><img src="${
                    company.logo || "/images/company-placeholder.svg"
                }" alt="logo"></div>
                <div class="hero-top" style="background-image:url('${
                    ...
                    <div class="hero-inner hero-container">
                        <div class="company-info-block">
                            ...
                            <div class="company-meta">
                                ${
                                    company.website
                                        ? `<span class="meta-item">🌐 <a href="${company.website}" target="_blank" style="color:inherit;opacity:0.95;text-decoration:underline">${company.website}</a></span>`
                                        : ""
                                }
                                ${
                                    company.employee_count
                                        ? `<span class="meta-item">👥 ${company.employee_count} nhân viên</span>`
                                        : ""
                                }
            ...
            <div class="container company-layout">
                ...
                <aside class="company-side">
                    <div class="card">
                        ...
                        <div class="contact-item"><div>📍</div><div><strong>Địa chỉ</strong><div>${
                            company.address || "Chưa có địa chỉ"
                        }</div></div></div>
                        <div class="contact-item"><div>✉️</div><div><strong>Email</strong><div>${
                            company.email || "---"
                        }</div></div></div>
                        <div class="contact-item"><div>📞</div><div><strong>Phone</strong><div>${
                            company.phone || "---"
                        }</div></div></div>
                        ${
                            company.website
                                ? `<div class="contact-item"><div>🔗</div><div><strong>Website</strong><div><a href="${company.website}" target="_blank">${company.website}</a></div></div></div>`
                                : ""
                        }
                    </div>
                </aside>
        ...
        `;
            ...
            if (breadcrumb) {
                breadcrumb.innerHTML = `Danh sách Công ty &nbsp;›&nbsp; Thông tin&nbsp; <strong>${
                    company.company_name || ""
                }</strong>`;
            }
        ...
        .catch((err) => {
            console.error(err);
            root.innerHTML =
                '<div class="alert alert-danger">Không thể tải thông tin công ty.</div>';
        });
});
    ...
    try {
        ...
        async function preloadSavedJobs() {
            try {
                ...
                list.forEach((item) => {
                    const jobId = Number(
                        item.job_id || item.jobId || (item.job && item.job.id)
                    );
        ...
        function createJobElement(job) {
            ...
            div.innerHTML = `
                ...
                companyName
            )}" onerror="this.onerror=null;this.src='/images/company-placeholder.svg'" loading="lazy">
                    </div>
                    <div style="flex:1">
                ...
                job.title || ""
                        ...
                        <div style="color:#6b7280;margin-top:6px;font-size:13px"><i class="fas fa-building"></i> ${escapeHtml(
                            companyName
                        )}</div>
                        <div class="job-meta-row" style="margin-top:8px">
                            <div class="meta-location"><i class="fas fa-map-marker-alt"></i> <span class="meta-text">${escapeHtml(
                                job.location || ""
                            )}</span></div>
                            <div class="meta-deadline"><i class="fas fa-calendar"></i> <span>Hạn nộp: ${escapeHtml(
                                deadlineText
                            )}</span></div>
                        </div>
                    </div>
                ...
                <div class="job-actions" style="display:flex;align-items:center;justify-content:space-between;margin-top:10px">
                    ...
                    <div style="display:flex;align-items:center;gap:8px">
                        <button class="btn-save ${savedClass}" data-job-id="${
                job.id
            }" onclick="toggleSaveJob(${job.id}, this)" title="Lưu việc làm">
        ...
        if (!jobs || jobs.length === 0) {
            jobsListEl.innerHTML =
                '<div style="padding:16px;color:#666">Chưa có việc làm nào từ công ty này.</div>';
            return;
        }
...
function normalizeLogoUrl(url) {
    ...
    try {
        ...
        if (!raw.match(/^https?:\/\//i) && !raw.startsWith("data:")) {
            ...
            return (
                window.location.origin + "/storage/" + raw.replace(/^\/+/, "")
            );
        }
