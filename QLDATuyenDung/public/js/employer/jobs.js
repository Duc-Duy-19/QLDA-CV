...
(function(){
    ...
    async function fetchMyCompany() {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE}/my-company`, { headers, credentials: 'include' });
            if (!res.ok) {
                myCompanyCache = null;
                return null;
            }
            ...
            try { console.debug('fetchMyCompany ->', myCompanyCache); } catch(e) {}
    ...
    async function fetchCategories() {
        ...
        try {
            const headers = Object.assign({'Accept':'application/json'}, token ? { 'Authorization': `Bearer ${token}` } : {});
            const res = await fetch(`${API_BASE}/public/categories`, { headers, credentials: 'include' });
            if (!res.ok) {
            ...
            let json;
            try { json = await res.json(); } catch(e) { json = null; }
            const data = json && (Array.isArray(json) ? json : (json.data && Array.isArray(json.data) ? json.data : null));
            categoriesCache = data || [];
            try { console.debug('fetchCategories ->', categoriesCache); } catch(e) {}
    ...
    function populateCategorySelect(categories) {
        try {
            ...
            if (!listWrapper) {
                ...
                if (expInput) {
                    ...
                    if (expRow && expRow.parentNode) {
                        ...
                        if (titleEl && titleEl.parentNode) titleEl.parentNode.insertBefore(wrapper, titleEl.nextSibling);
                        else return;
                    }
                } else {
                    const titleEl = qs('#job-title');
            ...
            categories.forEach(c => {
                ...
                label.style.gap = '8px';
                label.innerHTML = `<input type="checkbox" name="job-category[]" value="${id}" data-cat-id="${id}"> <span>${(c.name||c.title||('Danh mục '+id))}</span>`;
                listWrapper.appendChild(label);
            });
            // ensure there's an error container for category
            if (!qs('#job-category-error')){
                const err = document.createElement('div'); err.id = 'job-category-error'; err.className = 'field-error'; err.style.cssText = 'color:#dc2626;margin-top:6px;font-size:13px';
                listWrapper.parentNode && listWrapper.parentNode.appendChild(err);
            }
        } catch (e) { console.error('populateCategorySelect error', e); }
    }
    ...
    async function fetchPublicCompany(id) {
        ...
        try {
            const res = await fetch(`${API_BASE}/public/companies/${id}`, { headers: {'Accept':'application/json'}, credentials: 'include' });
            if (!res.ok) return null;
            let json;
            try { json = await res.json(); } catch(e){ json = null; }
            // public controller may return { company: {...} } or { data: {...} } or direct company object
            const company = (json && json.company) ? json.company : (json && json.data) ? json.data : json;
            publicCompaniesCache[id] = company || null;
            return publicCompaniesCache[id];
        } catch (e) { console.error('fetchPublicCompany error', e); publicCompaniesCache[id] = null; return null; }
    }
    ...
    function renderJobs(jobs) {
        ...
        if (!jobs || jobs.length === 0) {
            container.innerHTML = '<div class="empty-state">Chưa có tin tuyển dụng.</div>';
            return;
        }
        ...
        jobs.forEach(job => {
            ...
            let companyName = (job.company && (job.company.company_name || job.company.name)) ? (job.company.company_name || job.company.name) : (job.companyName || 'Công ty không xác định');
            let rawLogo = (job.company && (job.company.logo || job.company.avatar)) ? (job.company.logo || job.company.avatar) : null;
            let logoUrl = normalizeLogoUrl(rawLogo);
            try { console.debug('Employer job logo debug', { jobId: job.id, rawLogo: rawLogo, resolvedLogo: logoUrl }); } catch(e) {}
            div.innerHTML = `
                <div class="job-card-inner" style="display:flex;gap:16px;align-items:flex-start">
                    <div class="job-left" style="flex:0 0 auto">
                        <div class="job-logo-wrap" style="width:88px;height:88px;border-radius:12px;overflow:hidden;background:#f3f4f6;flex:0 0 88px;display:flex;align-items:center;justify-content:center">
                            <img src="${logoUrl}" alt="${escapeHtml(companyName)}" onerror="this.onerror=null;this.src='/images/company-placeholder.svg'" style="width:72px;height:72px;object-fit:contain">
                        </div>
                    </div>
                    <div class="job-right" style="flex:1;position:relative;min-width:0">
                        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px">
                            <div style="min-width:0">
                                ...
                                <div class="job-meta">${escapeHtml(companyName)}</div>
                            </div>
                            <div style="text-align:right;color:#16a34a;font-size:13px">${escapeHtml(job.location || '')}</div>
                        </div>
                        <div style="margin-top:8px;color:#6b7280">${escapeHtml(job.short_description || '')}</div>
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-top:12px">
                            <div class="job-salary">${escapeHtml(job.salary_range || '')}</div>
                            <div class="actions">
                                <button class="btn btn-ghost btn-view" data-id="${job.id}">Xem</button>
                                <button class="btn btn-ghost btn-toggle" data-id="${job.id}">${job.status === 'open' ? 'Đóng' : 'Mở'}</button>
                                <button class="btn btn-primary btn-edit" data-id="${job.id}">Sửa</button>
                                <button class="btn btn-danger btn-delete" data-id="${job.id}">Xóa</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            if ((!job.company || !(job.company.company_name || job.company.name || job.company.logo)) ) {
                (async function(){
                    try {
                        // try job.company.id or job.company_id or job.companyId
                        const cid = (job.company && (job.company.id || job.company.company_id)) || job.company_id || job.companyId || job.companyIdRaw || null;
                        let companyFromApi = null;
                        if (cid) {
                            console.debug('Attempting fetchPublicCompany for id:', cid);
                            companyFromApi = await fetchPublicCompany(cid);
                            console.debug('fetchPublicCompany result for', cid, companyFromApi);
                        }

                        if (companyFromApi) {
                            const nameEl = div.querySelector('.job-meta');
                            if (nameEl && (companyFromApi.company_name || companyFromApi.name)) nameEl.textContent = companyFromApi.company_name || companyFromApi.name;
                            const img = div.querySelector('.job-logo-wrap img');
                            if (img && (companyFromApi.logo || companyFromApi.avatar)) {
                                img.src = normalizeLogoUrl(companyFromApi.logo || companyFromApi.avatar);
                                img.alt = (companyFromApi.company_name || companyFromApi.name || img.alt);
                            }
                            return;
                        }
                        ...
                        if (!myCompany) return;
                        const nameEl = div.querySelector('.job-meta');
                        if (nameEl && (myCompany.company_name || myCompany.name)) nameEl.textContent = myCompany.company_name || myCompany.name;
                        const img = div.querySelector('.job-logo-wrap img');
                        if (img && (myCompany.logo || myCompany.avatar)) {
                            img.src = normalizeLogoUrl(myCompany.logo || myCompany.avatar);
                            img.alt = (myCompany.company_name || myCompany.name || img.alt);
                        }
                    } catch (e) { console.error('Error applying company info to job DOM', e); }
                })();
            }
        });
        ...
        qsa('.btn-delete').forEach(b => b.addEventListener('click', onDelete));
        qsa('.btn-toggle').forEach(b => b.addEventListener('click', onToggle));
    ...
    async function loadAndRender() {
        qs('#jobs-list').innerHTML = '<div class="empty-state">Đang tải...</div>';
    ...
    function openModal(mode='create'){
        ...
        qs('#modal-title').textContent = mode === 'create' ? 'Tạo việc làm' : 'Chỉnh sửa việc làm';
        if (mode === 'create'){
        ...
        (async function(){
            try {
                if (categoriesCache === undefined) {
                    ...
                    if (!qs('#job-category-list')) populateCategorySelect(categoriesCache || []);
                }
            } catch (e) { console.error('Error ensuring categories on openModal', e); }
        })();
    }
    function closeModal(){
    ...
    function clearForm(){
        ...
        const catChecks = document.querySelectorAll('input[name="job-category[]"]');
        if (catChecks && catChecks.length) catChecks.forEach(c => c.checked = false);
        const cat = qs('#job-category'); if (cat) cat.value = '';
    ...
    function clearJobFormErrors(){
        const ids = ['#job-title-error','#job-description-error','#job-requirements-error','#job-salary-error','#job-location-error','#job-expiration-date-error','#job-employment-type-error','#job-category-error'];
        ids.forEach(id => { const el = qs(id); if (el) el.textContent = ''; });
        const top = qs('#job-form-errors'); if (top) top.textContent = '';
    }
    ...
    function displayJobFieldErrors(errors){
        ...
        for (const key in errors){
            const arr = Array.isArray(errors[key]) ? errors[key] : [errors[key]];
            ...
            if (target){
                const el = qs(target);
                if (el){ el.textContent = arr.join(' '); if (!first){ const input = qs(target.replace('-error','')); if (input) input.focus(); first = true; } continue; }
            }
            unmapped.push(...arr);
        }
        if (unmapped.length){
            const top = qs('#job-form-errors');
            if (top) top.innerHTML = unmapped.map(m => `<div>${m}</div>`).join('');
    ...
    async function onSave() {
        ...
        const checkedCats = Array.from(document.querySelectorAll('input[name="job-category[]"]:checked')).map(i => String(i.value));
        ...
        try {
            ...
            if (!res.ok) {
                ...
                if (res.status === 422 && data && data.errors) {
                    try { displayJobFieldErrors(data.errors); } catch(e){ console.error('displayJobFieldErrors failed', e); }
                    return;
                }
                ...
                if (typeof txt === 'string' && txt.trim().startsWith('<')) {
                    throw new Error('Server returned HTML (có thể là trang đăng nhập). Vui lòng kiểm tra authentication.');
                }
    ...
    async function onEdit(e) {
        ...
        try {
            const headers = Object.assign({'Accept':'application/json'}, token ? { 'Authorization': `Bearer ${token}` } : {});
            const res = await fetch(`${API_BASE}/jobs/${id}`, { headers, credentials: 'include' });
            if (!res.ok) {
                const txt = await res.text(); console.error('Failed to fetch job detail:', res.status, txt);
            ...
            qs('#job-description').value = (job && job.description) ? job.description : '';
            qs('#job-requirements').value = (job && job.requirements) ? job.requirements : '';
            qs('#job-salary').value = (job && job.salary_range) ? job.salary_range : '';
            qs('#job-location').value = (job && job.location) ? job.location : '';
            qs('#job-employment-type').value = (job && job.employment_type) ? job.employment_type : 'Full-time';
            // set expiration date if available (normalize formats like "YYYY-MM-DD HH:MM:SS" or ISO with T)
            if (job && job.expiration_date) {
                const m = String(job.expiration_date).match(/(\d{4}-\d{2}-\d{2})/);
                if (m) qs('#job-expiration-date').value = m[1];
            }
            // set category if available (ensure categories/select exists first)
            try {
                console.debug('onEdit job payload', job, 'categoriesCache', categoriesCache);
                ...
                if (list && job) {
                    ...
                    if (Array.isArray(job.categories) && job.categories.length) {
                        ...
                        allChecks.forEach(c => { if (ids.includes(String(c.value))) c.checked = true; });
                    } else if (job.category_id) {
                        allChecks.forEach(c => { if (String(c.value) === String(job.category_id)) c.checked = true; });
                    } else if (job.category && job.category.id) {
                        allChecks.forEach(c => { if (String(c.value) === String(job.category.id)) c.checked = true; });
                    }
                }
            } catch (e) { console.error('error setting category on edit', e); }
            openModal('edit');
        } catch (err) {
            console.error(err);
    ...
    async function onDelete(e) {
        ...
        try {
            const headers = Object.assign({'Accept':'application/json'}, token ? { 'Authorization': `Bearer ${token}` } : {});
            const res = await fetch(`${API_BASE}/jobs/${id}`, { method: 'DELETE', headers, credentials: 'include' });
            let data;
            try { data = await res.json(); } catch(e){ data = null; }
    ...
    async function onToggle(e) {
        ...
        try {
            const headers = Object.assign({'Accept':'application/json'}, token ? { 'Authorization': `Bearer ${token}` } : {});
            const res = await fetch(`${API_BASE}/jobs/${id}/toggle-status`, { method: 'POST', headers, credentials: 'include' });
            let data;
            try { data = await res.json(); } catch(e){ data = null; }
            if (!res.ok) throw new Error((data && data.message) ? data.message : ('HTTP ' + res.status));
            await loadAndRender();
        ...
        (async function(){
            try {
                ...
                body.innerHTML = `
                    ...
                    <div class="job-meta">${escapeHtml(job.location || '')} • ${escapeHtml(job.employment_type || '')} • <strong>${escapeHtml(job.status || '')}</strong></div>
                    <hr>
                    <div class="detail-section"><strong>Mô tả</strong><div>${job.description ? job.description.replace(/\n/g,'<br>') : ''}</div></div>
                    <div class="detail-section" style="margin-top:8px"><strong>Yêu cầu</strong><div>${job.requirements ? job.requirements.replace(/\n/g,'<br>') : 'Không có'}</div></div>
                    <div class="detail-section" style="margin-top:8px"><strong>Mức lương</strong> <span class="salary-badge">${escapeHtml(job.salary_range || 'Thỏa thuận')}</span></div>
                `;
    ...
    document.addEventListener('DOMContentLoaded', function(){
        ...
        fetchCategories();
        qs('#btn-new-job').addEventListener('click', function(){ openModal('create'); });
        qs('#btn-cancel').addEventListener('click', function(){ closeModal(); });
        qs('#btn-save-job').addEventListener('click', function(){ onSave(); });
        qs('#job-modal').addEventListener('click', function(e){ if (e.target === qs('#job-modal')) closeModal(); });
    // detail modal close
    const detailModal = qs('#job-detail-modal');
    const detailClose = qs('#detail-close');
    const detailCloseFloat = qs('#detail-close-float');
    if (detailClose) detailClose.addEventListener('click', function(){ if (detailModal) detailModal.style.setProperty('display', 'none', 'important'); });
    if (detailCloseFloat) detailCloseFloat.addEventListener('click', function(){ if (detailModal) detailModal.style.setProperty('display', 'none', 'important'); });
    if (detailModal) detailModal.addEventListener('click', function(e){ if (e.target === detailModal) detailModal.style.setProperty('display', 'none', 'important'); });
