...
const hasApiHelper =
    ...
    window.APIHelper &&
    typeof window.APIHelper.request === "function";
    ...
    try {
        ...
        if (hasApiHelper) {
            try {
                ...
                console.warn(
                    "preloadFavoriteIds: APIHelper request failed, falling back to fetch",
                    e
                );
            }
        }
        ...
        if (!resp.ok) {
            ...
            console.warn(
                "preloadFavoriteIds: saved-jobs endpoint returned non-OK",
                resp.status,
                txt ? txt.slice(0, 200) : null
            );
            favoriteJobIdSet = new Set();
            return;
        }
        ...
        try {
            ...
            console.warn(
                "preloadFavoriteIds: failed to parse JSON from saved-jobs response",
...
async function preloadAppliedJobIds() {
    try {
            ...
            try {
                ...
                appliedJobIdSet = new Set(
                    (apps || [])
                        .map((a) =>
                            Number(
                                a.job_id || a.jobId || (a.job && a.job.id) || 0
                            )
                        )
                        .filter(Boolean)
                );
                ...
                console.warn(
                    "preloadAppliedJobIds: APIHelper request failed, falling back to fetch",
                    e
                );
            }
        }
        ...
        appliedJobIdSet = new Set(
            (apps || [])
                .map((a) =>
                    Number(a.job_id || a.jobId || (a.job && a.job.id) || 0)
                )
                .filter(Boolean)
        );
    } catch (e) {
...
async function loadCategoryJobs() {
    try {
        ...
        const categoriesList =
            categoriesJson && categoriesJson.data
                ? categoriesJson.data
                : categoriesJson;

        let categoryObj = (categoriesList || []).find(
            (c) =>
        ...
        const jobsRes = await fetch(
            `${API_BASE_URL}/api/public/categories/${categoryObj.id}/jobs?per_page=100`
        );
        ...
        allJobs = (jobsData || []).map((j) => {
            return Object.assign({}, j, {
                companyName:
                    (j.company && (j.company.company_name || j.company.name)) ||
                    j.company_name ||
                    j.companyName,
                logo:
                    (j.company && (j.company.logo || j.company.logo_url)) ||
                    j.logo ||
                    j.companyLogo,
        ...
        if (companyIdsToFetch.size > 0) {
            await Promise.all(
                Array.from(companyIdsToFetch).map((id) => fetchCompany(id))
            );
...
function showJobPopup(jobId, jobElement) {
    try {
        ...
        popup.innerHTML = `
            ...
            <p><strong>Công ty:</strong> ${escapeHtml(
                job.companyName || job.company_name || ""
            )}</p>
            <p><strong>Lương:</strong> ${escapeHtml(
            ...
            <div><strong>Mô tả:</strong> ${escapeHtml(
...
async function fetchCompany(companyId) {
    try {
        ...
        const res = await fetch(
            `${API_BASE_URL}/api/public/companies/${companyId}`
        );
        ...
        try {
            ...
            console.debug(
                "fetchCompany: response is not JSON for",
                companyId,
                "status",
                res.status
            );
            console.debug(
                "fetchCompany raw response snippet:",
                rawText && rawText.slice ? rawText.slice(0, 300) : rawText
            );
        ...
        if (company) {
            let logoCandidate =
                company.logo || company.logo_url || company.image || "";
            let normalized = "";
            try {
                if (logoCandidate) {
            ...
            try {
                const website =
                    company.website || company.website_url || company.site;
                if (website) {
...
function createJobElement(job) {
    ...
    const companyName = escapeHtml(
        job.companyName || job.company_name || "Công ty không xác định"
    );
    ...
    const deadlineText = formatDateToDMY(
        job.deadline || job.expiration_date || job.expiry || ""
    );
    const companyId =
        job.company_id ||
        job.companyId ||
    ...
    if (logoRaw && String(logoRaw).trim() !== "") {
        try {
            ...
            logoUrl = "/images/company-placeholder.svg";
        }
    } else if (companyId && companyCache.has(String(companyId))) {
        const c = companyCache.get(String(companyId));
        if (c) {
            if (c._logo_url && String(c._logo_url).trim() !== "")
                logoUrl = c._logo_url;
                ...
                try {
                    ...
                    logoUrl = `https://logo.clearbit.com/${u.hostname}`;
                } catch (e) {}
            }
        }
    } else if (job.company && job.company.website) {
        try {
            const u = new URL(job.company.website);
            logoUrl = `https://logo.clearbit.com/${u.hostname}`;
        } catch (e) {}
    }
    ...
    if (companyId && companyCache.has(String(companyId))) {
        ...
        if (cc && cc.website) {
            try {
                const uu = new URL(cc.website);
                fallbackUrls.push(`https://logo.clearbit.com/${uu.hostname}`);
            } catch (e) {}
        }
    }
    if (logoRaw) {
        try {
            if (/^https?:\/\//i.test(logoRaw) || logoRaw.startsWith("data:"))
                fallbackUrls.unshift(logoRaw);
            else if (logoRaw.startsWith("/"))
                fallbackUrls.unshift(window.location.origin + logoRaw);
            else
    ...
    if (companyId && companyCache.has(String(companyId))) {
        ...
        if (cc && cc.website) {
            try {
                const uu = new URL(cc.website);
                fallbackUrls.push(
                    `https://www.google.com/s2/favicons?domain=${uu.hostname}&sz=64`
                );
            } catch (e) {}
        }
    }
    ...
    return `
        ...
        companyName
    )}" data-website="${escapeHtml(
        job.website ||
            (companyId && companyCache.has(String(companyId))
                ? companyCache.get(String(companyId)).website ||
                  companyCache.get(String(companyId)).website_url
                : job.company && job.company.website) ||
            ""
    )}" onerror="${onerrorScript}" onload="console.debug('Logo loaded for', this.alt, 'src=', this.src)">
                </div>
                <div class="job-info">
                    <h4 class="job-title"><a href="/jobs/${
                        job.id
                    }" class="job-title-link" onclick="event.stopPropagation();">${escapeHtml(
        job.title || ""
    )}</a></h4>
                    ...
                    <div class="job-meta">
                        ...
                        <span><i class="fas fa-calendar"></i> Hạn nộp: ${escapeHtml(
                            deadlineText
                        )}</span>
                    </div>
                </div>
        ...
        job.id
    )}" onclick="toggleFavorite(${Number(job.id)}, this)" title="Lưu việc làm">
...
function updatePagination() {
    ...
    if (currentPage > 1) {
        paginationHTML += `<button class="page-btn" onclick="goToPage(${
            currentPage - 1
        })">
...
function checkCustomSalaryMatch(jobSalary) {
    ...
    const desiredSalary =
        parseInt(document.getElementById("salary-value").value) || 0;
        ...
        if (isFavorited) {
            if (hasApiHelper) {
                try {
                    if (
                        window.APIHelper &&
                        typeof window.APIHelper.ensureCsrf === "function"
                    ) {
                        await window.APIHelper.ensureCsrf();
                    }
                    const resp = await window.APIHelper.request(
                        `/saved-jobs/${jobId}`,
                        { method: "DELETE" }
                    );
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document
        ...
        .addEventListener("change", searchJobs);
});
