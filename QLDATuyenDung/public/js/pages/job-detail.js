...
async function loadJob(jobId) {
    try {
        ...
        if (!res.ok)
            throw new Error(
                "Không thể tải dữ liệu việc làm - Status: " + res.status
            );
        ...
        if (jobTitleEl) {
            jobTitleEl.textContent = job.title || "N/A";

            // Hiển thị badge nếu job đã đóng hoặc hết hạn
            const now = new Date();
            ...
            if (isExpired || isClosed) {
                ...
                badge.style.cssText =
                    "margin-left:15px;padding:6px 12px;background:#ef4444;color:white;font-size:14px;border-radius:6px;font-weight:600;";
                badge.textContent = isExpired ? "⏰ Đã hết hạn" : "🔒 Đã đóng";
                jobTitleEl.appendChild(badge);
            }
        }
        ...
        const companyName =
            (job.company && job.company.company_name) || job.company_name || "";
        const jobCompanySubEl = document.getElementById("job-company-sub");
        if (jobCompanySubEl)
            jobCompanySubEl.textContent =
                companyName + (job.location ? " • " + job.location : "");

        // meta pills
        const meta = [];
        if (job.salary_range || job.salary)
            meta.push({ text: job.salary_range || job.salary });
        if (job.location) meta.push({ text: job.location });
        if (job.experience || job.experience_level)
            meta.push({ text: job.experience || job.experience_level });
        if (job.expiration_date || job.deadline)
            meta.push({
                text:
                    "Hạn nộp: " +
                    formatDate(job.expiration_date || job.deadline),
            });
        ...
        if (jobDescEl) {
            ...
            if (job.description) {
                detailedHTML += `<div style="margin-bottom:24px;background:#f8fdf9;padding:20px;border-radius:8px;border:1px solid #d1f0db;">
                    ...
                    <div style="line-height:1.8;color:#1a1a1a;white-space:pre-wrap;font-size:15px;">${safeHTML(
                        job.description
                    )}</div>
                </div>`;
            }
            ...
            if (!detailedHTML) {
                detailedHTML =
                    '<div style="background:#fee;padding:30px;border:2px solid #f00;border-radius:8px;margin:20px 0;"><p style="color:#c00;font-size:18px;font-weight:bold;text-align:center;">⚠️ KHÔNG CÓ DỮ LIỆU MÔ TẢ</p></div>';
            }
            ...
            console.log(
                "Description HTML set successfully",
                detailedHTML.substring(0, 100)
            );
        } else {
        ...
        if (job.company && job.company.logo) {
            // Kiểm tra nếu là base64 data URL thì dùng trực tiếp
            if (job.company.logo.startsWith("data:image")) {
                resolvedLogo = job.company.logo;
                logoSource = "job.company.logo (base64)";
            } else {
        ...
        if (
            (!resolvedLogo || resolvedLogo === null) &&
            job.company &&
            job.company.id
        ) {
            try {
                const cRes = await fetch(
                    `${apiBase}/public/companies/${job.company.id}`
                );
                if (cRes.ok) {
                    ...
                    const cand =
                        ...
                        (cData.data &&
                            ...
                            cData.data.attributes.logo) ||
                        (cData.data &&
                            cData.data.attributes &&
                            cData.data.attributes.logo_url) ||
                        null;
                    if (cand) {
        ...
        if (!resolvedLogo) {
            resolvedLogo =
                window.location.origin + "/images/company-placeholder.svg";
        ...
        (function tryFallbacks() {
            ...
            const rawCandidates = [];
            if (job.company && job.company.logo)
                rawCandidates.push(job.company.logo);
            ...
            rawCandidates.forEach((rc) => {
                try {
                    ...
                    if (s && !/^https?:\/\//i.test(s)) {
                        if (s.startsWith("/"))
                            candidateUrls.push(window.location.origin + s);
                        else
                            candidateUrls.push(
                                window.location.origin +
                                    "/storage/" +
            ...
            try {
                if (company.id) {
                    ...
                    const cached =
                        ...
                        localStorage.getItem("company_cached_" + company.id) ||
                        localStorage.getItem("company_logo_" + company.id);
            ...
            if (company.website) {
                try {
                    ...
                    candidateUrls.push(
                        `https://logo.clearbit.com/${url.hostname}?size=128`
                    );
                    candidateUrls.push(
                    ...
                    [
                        ...
                        "/assets/logo.png",
                        "/images/logo.png",
                    ].forEach((p) => candidateUrls.push(base + p));
                } catch (e) {}
            }
        ...
        if (companyMetaEl)
            companyMetaEl.textContent =
                job.company && job.company.address ? job.company.address : "";
        ...
        return job; // Return job data for use in other functions
    } catch (err) {
        console.error(err);
        const jobTitleEl = document.getElementById("job-title");
        ...
        const jobDescEl = document.getElementById("job-description");
        if (jobDescEl)
            jobDescEl.textContent = err.message || "Vui lòng thử lại sau.";

        return null;
    }
}

// Sử dụng tên khác để tránh conflict với ai-analysis.js
let jobDetailResumes = null;
 ...
 */
function updateApplyButton(hasApplied, job = null) {
    ...
    if (job) {
        ...
        if (isExpired || isClosed) {
            btn.disabled = true;
            btn.textContent = isExpired ? "⏰ Đã hết hạn" : "🔒 Đã đóng";
...
async function handleApply(
    ...
    if (currentJob) {
        const now = new Date();
        const expirationDate = currentJob.expiration_date
            ? new Date(currentJob.expiration_date)
            : null;
        const isExpired = expirationDate && expirationDate < now;
        const isClosed = currentJob.status === "closed";

        if (isExpired || isClosed) {
            const messagesEl = document.getElementById("job-messages-fixed");
            if (messagesEl) {
                messagesEl.innerHTML = `<div style="color:#ef4444;font-weight:600;padding:12px;background:#fee2e2;border-radius:6px;margin:10px 0;">
                    ${
                        isExpired
                            ? "⏰ Công việc này đã hết hạn ứng tuyển."
                            : "🔒 Công việc này đã đóng."
    ...
    try {
        ...
        const coverLetter = coverLetterEl
            ? String(coverLetterEl.value).trim()
            : "";
        const sel = document.getElementById("resume-select");
        const selectedResumeId =
            sel && sel.style.display !== "none" ? sel.value || null : null;
        ...
        if (file) {
            ...
            console.log(
                "Applying with multipart form, keys:",
                ...
                if (res.status === 403) {
                    messagesEl.innerHTML =
                        data && (data.message || data.error)
                            ...
                            : "Bạn chưa có hồ sơ ứng viên hoặc không có quyền ứng tuyển.";
                    const note = document.getElementById("resume-note");
                    if (note) {
                        note.style.display = "block";
                        note.innerHTML = `Nếu chưa có CV, vui lòng <a href="${cvBuilderRoute}">tạo CV tại đây</a>.`;
                    ...
                    const msg =
                        data && (data.message || data.error)
                            ? data.message || data.error
                            : `Bạn đã ứng tuyển công việc này gần đây. Vui lòng thử lại sau.`;
                    ...
                    if (data && (data.message || data.errors || data.error)) {
                        ...
                        if (messagesEl)
                            messagesEl.innerHTML = `Ứng tuyển thất bại (status ${res.status})`;
                    }
                }
                if (btn) {
                    btn.disabled = false;
        ...
        if (
            !resumeId &&
            Array.isArray(jobDetailResumes) &&
            jobDetailResumes.length > 0
        ) {
            resumeId =
                jobDetailResumes[0].id || jobDetailResumes[0].resume_id || null;
        }

        if (!resumeId) {
            if (messagesEl)
                messagesEl.innerHTML = `Nếu chưa có CV, vui lòng <a href="${cvBuilderRoute}">tạo CV tại đây</a> trước khi ứng tuyển, hoặc tải lên file ở trên.`;
            if (btn) {
                btn.disabled = false;
                btn.textContent = "Ứng tuyển";
            }
        ...
        if (!res2.ok) {
            if (res2.status === 403) {
                messagesEl.innerHTML =
                    data2 && (data2.message || data2.error)
                        ? data2.message || data2.error
                        : "Bạn chưa có hồ sơ ứng viên hoặc không có quyền ứng tuyển.";
                const note = document.getElementById("resume-note");
                if (note) {
                    note.style.display = "block";
                ...
                data2 &&
                (data2.message || data2.errors || data2.error)
            ) {
                const errs = data2.errors
                    ? Object.values(data2.errors).flat().join(" - ")
                    : data2.message || data2.error;
                if (messagesEl) messagesEl.innerHTML = errs;
            } else {
                if (messagesEl)
                    messagesEl.innerHTML = `Ứng tuyển thất bại (status ${res2.status})`;
            }
            if (btn) {
                btn.disabled = false;
                btn.textContent = "Ứng tuyển";
        ...
        console.error(e);
        if (messagesEl)
            messagesEl.innerHTML = "Có lỗi xảy ra. Vui lòng thử lại.";
        if (btn) {
            btn.disabled = false;
            btn.textContent = "Ứng tuyển";
        }
    ...
    document.addEventListener("DOMContentLoaded", async function () {
        ...
        try {
            ...
            if (token) {
                ...
                updateApplyButton(hasApplied, job || currentJob);

                // Load resumes
                fetchAndPopulateResumes(token, routes.cvBuilder);
            } else {
                // Update button even when not logged in to show expired/closed status
                updateApplyButton(false, job || currentJob);
            }
        } catch (e) {
            /* ignore */
        }
        ...
        // Wire up AI analysis button - uses handleAiAnalysisHome from ai-analysis.js
        const aiAnalysisBtn = document.getElementById("ai-analysis-btn");
        if (aiAnalysisBtn) {
            aiAnalysisBtn.addEventListener("click", () => {
                if (typeof handleAiAnalysisHome === "function") {
                    handleAiAnalysisHome(jobId);
                } else {
                    alert(
                        "Chức năng AI chưa được tải. Vui lòng tải lại trang."
                    );
                }
            });
        }
...
// AI Analysis functions are now handled by ai-analysis.js
// No duplicate code needed here

// Export function to global scope
window.initJobDetailPage = initJobDetailPage;
